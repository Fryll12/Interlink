...
class KVIHelper:
        ...
        try:
            async with self.http_session.post(url, json=payload, timeout=8) as response:
                if response.status == 200:
                    ...
                    print(f"❌ [AI] Lỗi HTTP {response.status}")
                    return None
        except Exception as e:
            print(f"❌ [AI] Lỗi: {e}")
            return None
    ...
    async def create_suggestion_embed(self, kvi_data: Dict, ai_result: Dict) -> discord.Embed:
        ...
        suggestions = []
        for item in percentages[:min(3, len(kvi_data['choices']))]:
        ...
        embed.set_footer(text=f"🤖 Gemini AI • {len(kvi_data['choices'])} lựa chọn")
        return embed
    ...
    async def handle_kvi_message(self, message):
        ...
        try:
            ...
            print(f"❌ [DEBUG] Lỗi tải lại tin nhắn: {e}")
            return
        ...
        if not self.is_kvi_message(embed):
            ...
            return
        print("[DEBUG] Step 3: Tin nhắn là câu hỏi KVI hợp lệ.")
        ...
        # Tạo embed gợi ý
        print("[DEBUG] Step 7: AI trả kết quả, tạo embed gợi ý...")
